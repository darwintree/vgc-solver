...
`fixtureMs` 是构造输入时间；`prepareMs` 是规则审计和初始搜索准备；`searchMs` 是实际搜索时间；`totalMs` 从构造输入开始，到得到结果为止。进程启动、模块加载、JSON 写盘不计入 total。watchdog 从子进程创建计时。预算在转移之间检查，不能中断一次原生转移，所以实际搜索可能超时；这也是单独记录收敛与时间达标的原因。
