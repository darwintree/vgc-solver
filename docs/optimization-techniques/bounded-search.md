...
单击还可利用部分终局质量。原生伤害随机树给出首招 KO 质量 `[aL,aU]`（含首招命中）；对未 KO 的保留前缀，副作用条件概率上界为 `c`，回复命中下界为 `h`，回复命中后的 KO 质量下界为 `k`，于是回复终局质量至少 `(1-aU)(1-c)hk`。这些条件界逐层相乘，不假设独立；其余概率仍未知。Torrent 的高低 HP 区域及一次 Sitrus 回复分别用原生伤害 profile 与 HP 上界覆盖，事件槽/identity 审计及回退规则继续适用。
