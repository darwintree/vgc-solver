...
class TransitionPool {
  ...
  async start() {
    ...
    const startup = Array.from({length: this.size}, () => new Promise((resolve, reject) => {
      ...
      const slot = {worker, busy: false, starting: true};
  ...
  run(snapshot, p1Action, p2Action, options) {
  ...
  async initSolve(snapshot, {enabled = false} = {}) {
    await this.ready;
    ...
    const requests = this.workers.map(slot => new Promise((resolve, reject) => {
