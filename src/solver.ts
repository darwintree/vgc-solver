...
import type {ExactCell, ExactSolution, Snapshot, SolverOptions, StateKey} from './types';
...
class OneVsOneSolver {
  ...
  _snapshotKey(snapshot: Snapshot): string {
  ...
  _solve(snapshot: Snapshot, wantStrategy = false, eventPlan: ReturnType<typeof createEventPlan> = null): ExactSolution {
    ...
    try {
      ...
      const payoff: number[][] = Array.from({length: p1Actions.length}, () =>
        Array(p2Actions.length).fill(null)
      );
      const cells: ExactCell[][] = Array.from({length: p1Actions.length}, () =>
        Array(p2Actions.length).fill(null)
      );
