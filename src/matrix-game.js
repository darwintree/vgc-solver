 ...
 */
function simplexPositiveCover(matrix) {
...
function findPureSaddle(payoff) {
...
function validateEquilibrium(payoff, result) {
...
function solveSideByEnumeration(payoff, maximizing) {
  const rows = payoff.length;
  const columns = payoff[0].length;
  const size = maximizing ? rows : columns;
  const constraints = maximizing
    ? [...Array.from({length: rows}, (_, i) => ['zero', i]),
       ...Array.from({length: columns}, (_, j) => ['payoff', j])]
    : [...Array.from({length: columns}, (_, j) => ['zero', j]),
       ...Array.from({length: rows}, (_, i) => ['payoff', i])];
  let best = null;
...
function solveByEnumeration(payoff) {
