...
class AsyncTransitionSolver {
  ...
  async _solve(snapshot, wantStrategy = false, ancestors = new Set()) {
    this.checkAbort();
  ...
  async _solveBody(snapshot, wantStrategy, key, ancestors) {
    this.checkAbort();
    ...
    const resolveCell = async (i, j) => {
      ...
      const childValues = new Map(await Promise.all([...childPromises.entries()]
        .map(async ([childKey, promise]) => [childKey, await promise])));
      this.checkAbort();
      for (let index = 0; index < cell.outcomes.length; index++) {
...
module.exports = {AsyncTransitionSolver};
