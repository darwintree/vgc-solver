...
class BoundedSolver {
  ...
  _intern(snapshot, skipTerminal = false) {
  ...
  _acceptTransition(node, i, j, transition) {
  ...
  _selectFrontier(root, selectionPolicy = this.options.selectionPolicy) {
    const path = new Set();
    let node = root;
