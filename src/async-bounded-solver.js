...
const {snapshotBattle, stateKey} = require('./showdown-adapter');
const {createMemoStateKey} = require('./native-memo-key');
const {auditPPBattle} = require('./pp-transition-cache');
const {auditNativeRules} = require('./native-rules');
const {compareStockProfile} = require('./stock-rule-profile');
 ...
 * unchanged.
 */
class AsyncBoundedSolver extends BoundedSolver {
  constructor(options = {}) {
  ...
  async solve(battle) {
    ...
    try {
      ...
      if (!this.usesNativeAdapter) {
        const result = super.solve(battle);
...
module.exports = {AsyncBoundedSolver};
