import type {ID, Pokemon, PokemonSet} from '@pkmn/sim';
...
import {snapshotBattle, restoreBattle, stateKey} from '../src/showdown-adapter';
...
function makeBattle(formatid = 'gen9customgame') {
  ...
  const battle = new Battle({
    formatid: formatid as ID, debug: true, strictChoices: true,
    p1: {name: 'P1', team: [
...
test('snapshot detaches only native serializer aliases and preserves state shape', () => {
  ...
  } as unknown as Pokemon['volatiles'][string];
...
test('shared team set aliases preserve old JSON roundtrip splitting', () => {
  const battle = makeBattle();
  (battle.p1.pokemon[1] as Mutable<Pokemon>).set = battle.p1.pokemon[0].set;
  const snapshot = snapshotBattle(battle);
