...
const {BranchingPRNG, NeedRandom} = require('../src/branching-prng');
const {assertSameDistribution, enumerateNative} = require('./helpers/distribution');
const {installSimulatorOptimizations} = require('../src/simulator-optimizations');
const {
  createBattle,
  enumerateTurn,
  snapshotBattle,
} = require('../src/showdown-adapter');
const {setHP, refreshMoveRequest} = require('../src/showdown-adapter');
...
test('secondary aggregation bypasses forceRandomChance like native random(100)', () => {
  ...
  nativeBattle.forceRandomChance = true;
  nativeBattle.prng = new BranchingPRNG();
  ...
  optimizedBattle.forceRandomChance = true;
  optimizedBattle.prng = new BranchingPRNG();
...
function compareDamage(move, targetOptions = {}, hp = 1) {
  const battle = createBattle(
...
function compareLowHP(move, targetOptions = {}) {
