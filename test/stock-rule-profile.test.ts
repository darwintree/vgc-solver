import type {ID, PRNGSeed, PokemonSet} from '@pkmn/sim';
...
function clearRuntimeCaches() {
  ...
  ] as const) data[cacheName]?.clear?.();
}

function startBattle(p1, p2) {
  const battle = new Battle({
    formatid: 'gen9customgame' as ID, debug: true, strictChoices: true,
    seed: [1, 2, 3, 4] as unknown as PRNGSeed, p1: {name: 'P1', team: [p1] as PokemonSet[]}, p2: {name: 'P2', team: [p2] as PokemonSet[]},
  });
...
test('a stock format numeric mutation is detected before battle setup', () => {
  const format = Dex.formats.get('gen9customgame') as Mutable<ReturnType<typeof Dex.formats.get>>;
  const original = format.playerCount;
  (format as {playerCount: number}).playerCount = 99;
  try {
...
test('a constructed numeric mutation disables stock sharing and changes trivialPriorityKO', () => {
  ...
  const move = Dex.moves.get('quickattack') as Mutable<ReturnType<typeof Dex.moves.get>>;
...
test('a known native function copied into another raw definition is rejected', () => {
  const tackle = Dex.moves.get('tackle') as Mutable<ReturnType<typeof Dex.moves.get>>;
...
test('species numeric mutations, Map replacements, and allCache order are guarded', () => {
  ...
  moveCache.set('tackle' as ID, replacement);
  try {
    assert.equal(compareStockProfile(Dex, profile).ok, false);
  } finally {
    moveCache.set('tackle' as ID, originalMove);
  }

  const all = Dex.moves.allCache as Mutable<typeof Dex.moves.allCache>;
  const first = all[0];
  const second = all[1];
...
test('a copied known native callback disables sharing and native fallback observes it', () => {
  const soundproof = Dex.abilities.get('soundproof') as RuntimeAbility;
  const sturdy = Dex.abilities.get('sturdy') as RuntimeAbility;
