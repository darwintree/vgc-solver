import type {MatrixSolution} from '../src/types';
...
function normalize(strategy: number[]): number[] {
  const sum = strategy.reduce((a, b) => a + b, 0);
  return strategy.map(x => x / sum);
}

function solveSide(payoff: number[][], maximizing: boolean) {
  const m = payoff.length;
  const n = payoff[0].length;
  const size = maximizing ? m : n;
  const constraints = maximizing
    ? [...Array.from({length: m}, (_, i) => ['zero', i] as const),
       ...Array.from({length: n}, (_, j) => ['payoff', j] as const)]
    : [...Array.from({length: n}, (_, j) => ['zero', j] as const),
       ...Array.from({length: m}, (_, i) => ['payoff', i] as const)];
  let best = null;
...
function solveZeroSumMatrixOracle(payoff: number[][]): MatrixSolution {
...
export {solveZeroSumMatrixOracle};
