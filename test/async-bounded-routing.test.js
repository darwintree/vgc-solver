...
const {AsyncBoundedSolver} = require('../src/async-bounded-solver');
...
test('bounded worker routing gives distinct row-major affinity for variable action grids', async () => {
  for (const [rows, columns] of [[2, 3], [3, 2], [2, 4]]) {
    ...
    const solver = new AsyncBoundedSolver({pool, workerCount, batchSize: workerCount});
    ...
    await solver._expandCellsAsync(node, cells);
